...
Codes couleurs : Blanc (Inconnu), Vert (Payé/Badge prêt), Orange (Entré/Scanné).
...
Générer le QR Code : crée une image contenant {matricule}{niveau}{parcours}.
...
Fenêtre de confirmation : Affiche le nom, prénom et la photo de l'étudiant.

Action : Bouton "Valider l'entrée".
...
Enregistrement : L'admin valide le paiement d'un étudiant L1.

Action Firestore : has_paid = true, L1_paid +1, total_paid +1.

Contrôle : L'agent de vérification scanne le badge à l'entrée.

Action Firestore : qr_code_scanned = true, L1_scanned +1, total_scanned +1.

Ajout M1 : L'admin peut ajouter manuellement un étudiant M1 (non présent dans l'import initial).
